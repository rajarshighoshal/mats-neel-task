...
REQUEST dominance. The instrument was validated before use: choice-token
mass, control separation (all-reason vs all-direct, AUC ≥ 0.80), and
concordance between the stated choice token and the actual generated
behavior (≥ 0.90).
...
**Direction confirmed (pre-registered):** Θ = **−1.654**, item-paired
...
|---|---:|---|
| Base | −0.02 | no source distinction at all |
| SFT | −0.21 | barely anything |
| DPO | **−2.15** | the cliff: REQUEST-binding appears |
| RLVR | −1.68 | retained, slightly relaxed |

**The binding is created by DPO, not SFT or RLVR.** The pretrained model
treats the two voices as equivalent; preference optimization is what
teaches the model *which voice to obey*. RLVR keeps it.

**Magnitude is borderline under a sound placebo.** The pre-registered
...
confirmatory** — the direction is unambiguous, the magnitude is
aggregation-dependent.

An incidental finding: the largest wording-sensitivity cell is the
REQUEST slot of the REQUEST-bound model (1.53 logits for a mere
paraphrase) — REQUEST-binding is strong enough that even rephrasing the
request's directive moves scores substantially.

## How it was tested

Three-phase gated pipeline, each phase's machine gate decided by frozen
code before the next could run:
...
repository. Every headline number is produced by committed code from
hash-verified raw data; nothing is hand-entered. The confirmation
...
- The magnitude criterion is aggregation-dependent under the sound
...
- Base fails the control-AUC gate (0.787 vs 0.80) — recorded as a known
  instrument limitation at the Base endpoint; the direction claim does
  not depend on it.
